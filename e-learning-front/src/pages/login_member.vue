<template>
  <div class="bg">
    <div class="top"><span>会员登录入口 / </span><span class="to_member" @click="goTo">去职员登录入口</span></div>
    <div class="container">
      <div class="left">
        <p>注册</p>
        ...
        <el-input placeholder="请输入用户名" v-model="user_name" maxlength="10" show-word-limit>
          <template slot="prepend"><img src="../assets/iconfont/friends.png"></template>
        </el-input>
        <el-input placeholder="请输入邮箱" v-model="e_mail" type="email">
          <template slot="prepend"><img src="../assets/iconfont/message.png"></template>
        </el-input>
      ...
      <div class="right">
        ...
        <el-button round @click="visitor">游客浏览</el-button>
      </div>
    </div>
  </div>
</template>
...
<script>
import {loginMember, regMember} from '@/api/api'
export default {
  name: 'login_staff',
  data () {
    return {
  ...
  methods: {
    ...
    login () {
      if (this.phone && this.password) loginMember(this.phone, this.password)
      else alert('请填写完整！')
    },
    reg () {
      if (this.phone_num && this.user_name && this.e_mail && this.pass_word1 && this.pass_word2) {
        if (this.pass_word1 === this.pass_word2) {
          if (this.pass_word1.length >= 6) regMember(this.phone_num, this.user_name, this.e_mail, this.pass_word1)
          else alert('密码长度少于6位！')
        } else alert('两次密码输入不同！')
      } else alert('请填写完整！')
    }
...
<style scoped>
  .bg{
    ...
    height: 100%;
    padding: 10%;
    background-image: url("../assets/img/mem_bg.jpg");
    background-size: 100%;
